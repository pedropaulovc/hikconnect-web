...
import { cameraById, eventsForCamera, recordingsForCamera } from './index';
...
describe('selectors', () => {
  ...
  it('recordingsForCamera returns only that camera', () => {
    const id = cameras[0].id;
    expect(recordingsForCamera(id).every((r) => r.cameraId === id)).toBe(true);
  });
});
